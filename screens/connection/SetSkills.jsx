...
function SetSkills({ navigation }) {
  ...
  const [skills, setSkills] = useState(
    postesTravaux.reduce((acc, poste) => {
      acc[poste] = null;
      return acc;
    }, {})
  );
  ...
  const handleSaveSkills = async () => {
    ...
    if (response.status === 201) {
      ...
      const addSkillsResponse = await fetch(
        `${ipString}/users/addSkillsToUser`,
        {
          method: "PUT",
          headers: {
            Authorization: token,
          ...
          body: JSON.stringify({ token, skillsId }),
        }
      );

      const addSkillsData = await addSkillsResponse.json();
      console.log(
        "Réponse de la mise à jour des compétences de l'utilisateur:",
        addSkillsData
      );
